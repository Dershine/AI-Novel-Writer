...
2. 允许直接叙述、概述、心理判断和符合视角的简短评述。保留能增加因果、人物偏见、反讽或理解角度的解释；删去只把前文动作或对白换一种说法的解释。不用拔剑声、喝水、目光变化等装饰替代需要交代的信息，不在结尾添加与正文无关的哲理感悟。
3. 人物根据自己的知识、经历和当下需要注意不同的事。在重要选择处，让读者理解人物依据什么作出判断。区分人物所知的事实、猜测和愿望，不把人物自信自动写成判断正确，也不将作者明确事实降格为猜测。
4. 对白由人物当下想获得、隐瞒、拒绝或确认的东西产生。根据双方关系决定哪些话直说、哪些话绕开，不要求句句都有潜台词。不让所有人都以同样机敏、体贴或完整的方式回答。幽默应有可理解的缘由，不靠“大家笑了”证明笑点成立。
...
2. 叙述取舍：根据本章内容的重要性选择场景展开或简洁概述；在关键处写清人物的观察、判断与应对，在过渡处省略重复过程，不为制造动态而堆砌神态、动作和声音。
