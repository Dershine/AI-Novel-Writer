...
const DRAFT_PROSE_GUIDANCE = `【叙述与表达】
1. 根据内容的重要性分配篇幅。改变局势、人物判断或关系的关键时刻，用具体场景展开；重复过程、日常往返和已确立的相处模式可以简洁概述。不为达到字数补充无效动作、寒暄或重复心理。
2. 允许直接叙述、概述、心理判断和符合视角的简短评述。解释应增加新的因果、人物偏见、误判、私心、反讽或理解角度；若只是重复已成立的信息，或替人物证明其行为合理、周全、没有别的意思，应压缩或删除。人物的自我解释可以保留，但叙述不必自动认可，更不必替矛盾作最后定性。不用装饰动作替代需要交代的信息，不在结尾添加与正文无关的哲理感悟。
3. 人物的注意力和语言应受到其知识、经历、欲望、偏爱、自尊与当下情绪影响。在影响下一步行动的重要选择处，交代足以理解选择的依据；选择成立后继续推进，不必逐项排除所有风险与其他可能。区分人物所知的事实、猜测和愿望，但不将每次判断写成知识边界说明。不把人物自信自动写成判断正确，也不将作者明确事实降格为猜测。
4. 对白由人物此刻想对另一个人做什么产生，例如争取、拒绝、遮掩、试探、亲近、炫耀或发泄。人物说多少、漏掉什么、如何接话，取决于身份、性情与关系。不要求句句有潜台词，也不默认双方会立即消除所有误会、尴尬或失落。不让所有人都以同样机敏、体贴或完整的方式回答。幽默应有可理解的缘由，可以暴露性格、改变关系，不必每次都让双方体面地接住；笑点成立后不再解释，不靠“大家笑了”证明笑点成立。
5. 动作、环境和感官细节应帮助读者理解处境、人物或气氛。不在每句对白后补微笑、点头、蹙眉、停顿。有效细节可以承担多种作用，但不必将每处描写都设计成伏笔。
6. 段落按完整意思和注意力变化组织，长短自然交替。短句、独立段用于需要强调之处，不默认一句一段。修辞以准确、贴切为准，删去重复或无实际作用的意象。
7. 输出前检查关键场景：解释是否只是复述刚才已经成立的意思；人物的回应是否过于一致、总能恰到好处；最有力量的动作或话语之后，是否又补了一段把它解释平的文字。优先通过删减、前移说明、调整回应和段落解决，不靠新增冲突、笑话或情节补救。保留理解因果所必需的信息，直接输出修改后的正文。`
...
{{pacing_guidance}}`,
...

  {
    ...
    systemRole: '你是一位经验丰富的小说作者。尊重作者事实与叙述视角，根据情节需要选择场景展开、简洁概述、人物判断和有区分度的对话，建立清楚的因果与阅读期待，不输出思考过程或元话术。',
    variables: {
...
【第一章创作要求】
1. 开篇尽快建立一个具体人物的处境，以及值得继续阅读的问题、愿望、不协调之处或关系。可以从行动、对白、人物状态或简短环境切入，选择最适合本章事件的入口。不要为制造高能开场擅自新增危险，也不要在读者尚未产生兴趣前集中介绍世界观。
2. 仅当【本章信息】明确要求时才展现主角的金手指；不得为满足通用套路擅自新增事件。
3. 视角内推进：通过符合当前视角的叙述、动作、感官、内心活动和对话推动剧情；不得仅为展示信息而让角色公开说出只由其私下感知、尚未转述的内容。
4. 落实全局写作要求，避开其中列出的写作问题。
...
- 作者明确事实与指导、实际前文、本章关键因果和本章篇幅优先。必须保留作者指定的事实、关键事件、因果、人物关系阶段和结束状态，不得提前执行后续情节；不得把作者明确事实或要求降格为推测。
- 在保留作者事实、本章关键事件、人物关系阶段和结束状态的前提下，自主安排叙述、对白与反应。事实需要准确，不等于人物必须把话说得准确周全；角色可以隐瞒、嘴硬、答非所问、没有想到某件事，叙述也不必认可角色对自己的解释。这些表达应来自既有性格与当下处境，不为制造戏剧性强加失误、争执或新事件。正文无需解释自己为何符合设定。

★【作者对本步骤的额外指导（如有，最高优先级）】★：
{{user_guidance}}

【具体生成要求】
- 体量与节奏控制：大约 {{word_number}} 字左右。本章仅推演【本章信息】中规定的核心剧情。按【本章信息】约定的结束状态或悬念收束；未明确要求时自然断章，绝不可提前泄露后续情节。
...
${DRAFT_PROSE_GUIDANCE}`,
...

  {
    ...
    systemRole: '你是一位经验丰富的小说作者。保持作者事实、叙述视角与长篇连续性，根据情节需要选择场景展开、简洁概述、人物判断和有区分度的对话，呈现本章的因果与变化，不输出思考过程或元话术。',
    variables: {
      architecture: '故事架构（故事前提+角色图谱+世界观+情节大纲）',
      novel_config: '作者确认的小说配置（权威约束）',
...
【网文连载更新核心法则】
1. 向前推进：【剧情记忆库与前置断点上下文】和【上一章已完成的结尾状态】记录的是已发生历史；【本章写作方向与核心任务】、【后续章节大纲预告】和【知识库资料】不因此成为已发生事件。承接前文已经形成的局面，让本章产生新的进展。可以简短回答上章留下的问题、交代必要的时间地点变化，或呼应具有新含义的细节；不要重复演出已经完成的事件。若本章蓝图要求倒叙、跳时或切换视角，应清楚建立转换后的时空与视角。
2. 叙述取舍：优先展开使人物的预期、判断、处境或关系发生变化的时刻。常规流程写到足以理解即可；关键处既要让读者知道发生了什么，也要感到这件事对当事人意味着什么。在过渡处省略重复过程，不为制造动态而堆砌神态、动作和声音。
